...
angular.module('trikatuka2').factory('User', function (Spotify, $sessionStorage, $rootScope, $http, $q) {
    ...
    User.prototype.login = function () {
        ...
        ipcRenderer.send('login', self.id === 'user1' ? 'SOURCE_USER' : 'TARGET_USER')
        ipcRenderer.once('login-response', (event, data) => {
            self.authData = data;
    ...
    User.prototype.refreshToken = function () {
        ...
        ipcRenderer.send('refresh-token', self.id === 'user1' ? 'SOURCE_USER' : 'TARGET_USER')
        ipcRenderer.once('refresh-token-response', (event, data) => {
            self.authData = data;
            self.authData.expiresAt = new Date().getTime() + (self.authData.expires_in * 1000);
            $sessionStorage[self.id] = self.authData;
